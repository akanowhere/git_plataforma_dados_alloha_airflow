...
-----------------------------------------
---- DADOS FATURAS VENCIMENTO DENTRO DO MÊS
-----------------------------------------

dados_faturas_venc_mes AS (
...
A.valor_fatura,
A.valor_pago,
DATEDIFF(DAY, A.data_vencimento, A.data_pagamento) AS aging_pagamento,
CASE
	WHEN UPPER({{ translate_column('A.status_fatura') }}) IN  ('NEGOCIADA COM O CLIENTE',
                                                                'FATURA MINIMA',
                                                                'ABONADA',
                                                                'CANCELADA') THEN NULL
	WHEN DATEDIFF(DAY, A.data_vencimento, A.data_pagamento) <= 0 THEN 'BOM PAGADOR'
	WHEN DATEDIFF(DAY, A.data_vencimento, A.data_pagamento) BETWEEN 1 AND 5 THEN 'BAIXO RISCO'
	WHEN DATEDIFF(DAY, A.data_vencimento, A.data_pagamento) BETWEEN 6 AND 15 THEN 'MEDIO RISCO'
	WHEN DATEDIFF(DAY, A.data_vencimento, A.data_pagamento) > 15 THEN 'ALTO RISCO'
	WHEN DATEDIFF(DAY, A.data_vencimento, A.data_pagamento) IS NULL THEN 'ALTO RISCO'
ELSE NULL END comportamento_mensal

FROM {{ ref('dim_faturas_mailing') }} A

where YEAR(A.data_vencimento) = YEAR(DATEADD(DAY, -1, DATEADD(HOUR, -3, current_date())))
AND MONTH(A.data_vencimento) = MONTH(DATEADD(DAY, -1, DATEADD(HOUR, -3, current_date())))
)

-----------------------------------------
----Criação perfil de clientes analítico
-----------------------------------------
