SELECT
  ...
  COUNT(id_contrato) AS qtd_ativos,
...
FROM
  (
    ...
    FROM {{ get_catalogo('silver') }}.stage_contrato.vw_situacao_contrato AS x
...
GROUP BY
  ...
  canal,
  marca,
  UPPER(base.unidade_atendimento),
  regional,
  produto,
  pacote_tv,
  tipo_tv,
...
nome_regional
FROM {{ this }}
WHERE data_referencia < DATE_SUB(CURRENT_DATE(), 1)
