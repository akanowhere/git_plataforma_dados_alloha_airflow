SELECT DISTINCT 
      ...
      max(cast(w.send_at as TIMESTAMP)) DataDisparo,
...
LEFT JOIN {{ get_catalogo('gold') }}.infobip.dim_os_notificacao_confirmacao C ON C.message_id = W.message_id

LEFT JOIN {{ get_catalogo('gold') }}.chamados.dim_ordem O ON O.codigo_os = C.id_os

LEFT JOIN (
    SELECT
      DISTINCT a.id,
      CAST(a.data_confirmacao_agendamento AS TIMESTAMP) AS datahora_confirmacao_agendamento,
      CAST(a.data_confirmacao_agendamento AS DATE) AS data_confirmacao_agendamento,
      a.motivo_conclusao
    FROM
      {{ get_catalogo('silver') }}.stage.vw_air_tbl_os a
) CA ON CA.id = C.id_os

LEFT JOIN (
    SELECT
      DISTINCT CAST(data_criacao AS DATE) Data,
      id,
      servico,
      observacao
    FROM
...
WHERE
  ...
  AND CA.data_confirmacao_agendamento is NOT NULL
