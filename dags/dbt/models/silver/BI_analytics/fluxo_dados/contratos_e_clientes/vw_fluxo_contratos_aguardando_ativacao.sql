...
FROM
  ...
  INNER JOIN {{ get_catalogo('gold') }}.base.dim_unidade t2 ON t1.unidade_atendimento = t2.sigla
  AND t2.fonte = 'AIR'
  LEFT JOIN {{ get_catalogo('gold') }}.base.dim_cliente t4 ON t1.id_cliente = t4.id_cliente
  AND t4.fonte = 'AIR'
  LEFT JOIN (
    SELECT
      id_contrato,
  ...
  LEFT JOIN {{ get_catalogo('gold') }}.base.dim_endereco t6 ON t5.id_endereco = t6.id_endereco
  LEFT JOIN {{ get_catalogo('gold') }}.base.fato_cancelamento t7 ON t1.id_contrato_air = t7.CODIGO_CONTRATO_AIR
  LEFT JOIN {{ get_catalogo('gold') }}.base.dim_catalogo t8 ON t1.status = t8.codigo
  AND t8.ativo_catalogo = TRUE
WHERE
  t1.status = 'ST_CONT_EM_ATIVACAO'