...
from config.data_contract_reader import AppContractReader
...
pipeline_options["APP_AUTH_FIREBASE"] = Variable.get("APP_AUTH_FIREBASE")


# Obtendo os parâmetros das camadas landing e bronze
data_contract = AppContractReader(
...
@dag(
    dag_id="Ingestion_daily_app_pipeline",
    default_args={
    ...
    catchup=False,
    tags=["app", "daily", "job_databricks", "landing", "bronze"],
)
def app_dag() -> None:
    ...
    to_gold_tasks = AirflowUtil().create_dbt_task_group(
        ...
        schema="app",
        select_paths=["path:models/gold/app"],
        exclude=[""],
    )
